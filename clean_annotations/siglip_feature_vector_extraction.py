...
def extract_features_from_bbox(
    ...
    # Get the coordinnates of the bounding boxe, et préparation de l'image d'input 
    ...
    if input_cell : 
        if i  == 0 : 
            print("shape de cell_image avant resize", cell_image.size)
        cell_image = cell_image.resize((size_img_new,size_img_new)) # resize par upsample de l'image de la cellule pour correspondre aux taille d'entrées du modèle
        if i == 0 : 
            print("shape de cell_image après resize", cell_image.size)
        input_img = cell_image
    else : 
        input_img = img_cropped
    ...
    feats = feats.reshape(1, num_patches, num_patches, dim_feature_vector)
    if i == 0 : 
        print("shape des features reshape", feats.shape)
    features = feats.squeeze(0).cpu().numpy()  
    if input_cell : 
        avg_feature = features.reshape(num_patches*num_patches,dim_feature_vector).mean(axis=0)
        if i == 0 : 
            print("shape de avg feature", avg_feature.shape)
    else : 
        ...
        # Downsample the segmentation to match the model feature map size
        seg_downsampled = cv2.resize((seg_crop * 255).astype(np.uint8), (num_patches, num_patches), interpolation=cv2.INTER_LINEAR)
        ...
        if masked_features.size == 0:
            ...
            return np.zeros(dim_feature_vector)
        if i == 0 : 
            print("shape de masked feature", masked_features.shape)
...
def main(bbox_file, input_size, input_cell=True, model_name="vit_large_patch16_siglip_gap_512.v2_webli", dstdir="./features_vectors/", device="cuda"):
    """Re-run Dino on bounding boxes extracted from file and save results as .npy."""
    # 1. --- Load du modèle --- 
    siglip_models = [m for m in timm.list_models() if "siglip" in m]
    print(f"siglips models disponible avec timm : {siglip_models}")
    ...
    print("Img transform relié au modèle", img_transform)
    ...
    for i, bbox in enumerate(tqdm.tqdm(bboxes, desc="Processing bounding boxes", unit="bbox")):
        ...
        if i == 0 : 
            ...
            if input_cell : 
                output_file = dstdir / f"X_labeled_{model_name}_input_size_{size_img_new}_input_cell_std_0_5_mean_05.npy"
            else : 
                output_file = dstdir / f"X_labeled_{model_name}_input_size_{size_img_new}_std_0_5_mean_05.npy"
        feature_vector = extract_features_from_bbox(image, bbox_coord, img_transform, model, device, size_img_new, i, num_cls_tokens, patch_size, dim_feature_vector, input_cell)
